...
后台服务管理、云文档评论、COT 消息、/config 卡片、secrets 加密、`/invite all group` 批量列举；
...
Codex 外部 CLI/桌面会话可由 `/sessions` 查看状态与最近输出、由 `/resume` 续接；活动 turn 仅在 shared daemon 中使用 `turn/steer` 安全接管（ADR-0025）。
